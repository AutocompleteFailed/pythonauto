...
class GDBInterface:
    def __init__(self, logger, exec_path):
        self.logger = logger
        self.logfile = open('/tmp/automarker_gdb.log', 'a')
        #self.gdb = pexpect.spawnu("arm-none-eabi-gdb", timeout=10, logfile=self.logfile)
        self.gdb = pexpect.spawn("gdb-multiarch {}".format(exec_path), timeout=10, logfile=self.logfile)    #, encoding='utf-8'   , logfile='sys.stdout'
    ...
    def connect(self, host="localhost", port="1234"):
        cmd = "target remote {}:{}".format(host,port)
        self.gdb.sendline(cmd)       # modified a little to take host and port as arguments
        expect_str = ".*using {}:{}[\s\S]*\([\s\S]*\)[\s\S]*\(gdb\)".format(host,port)
        if self.gdb.expect([expect_str, \
