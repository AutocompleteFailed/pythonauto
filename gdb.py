...
class CommandResult:
    ...
    def get_stream_messages_text(self):
        ...
        :rtype: str
        """
        stream_messages_text = "".join([m.value for m in self.stream_messages])
        print("%s: %s",end='') % ( self.timestamp, stream_messages_text)
        return stream_messages_text
...
class GDB:
    ...
    def __init__(self, exec_path, path='/usr/bin/gdb', *extra_args):  # pylint: disable=W1113
    ...
    def read_until_break(self, max_lines=100):
        ...
        while True:
            ...
            if line in GDB_BREAK_CONDITIONS:
                #print("Line has matched a break condition: "+ str(line), end='')
                break
            #if is_break_hit(decoded_line):
            #    print("is_break_hit() has returned true",end='')
            #    break
            if len(result) >= max_lines:
                break
            result.append(line)
    ...
    def get_variable_value(self, var):
        """
        Read the value of a variable if it exists.

        :param var: variable we want the value of
        :type var: str
    ...
    def read_from_address(self, address):
        """
        Reads the value at an address in memory.
        ...
        cmd = "x %s" % address
        r = self.cmd(cmd)
        if r.result.type == None:
            print("result type is none")
        if not r.result.class_ == 'done':
            raise UnexpectedResponseError
        return r
    ...
    def cmd(self, command):
        ...
        for line in responses:
            ...
            try:
                ...
                #if is_break_hit(parsed_response):
                #    print("is_break_hit returned true inside cmd()",end='')
            except Exception:  # pylint: disable=W0703
                cmd.application_output.append(line)
                print("[GDB terminal] "+str(line).rstrip(),end='')
                continue

            if (parsed_response.type == 'console' and
                    parsed_response.record_type == 'stream'):
                cmd.stream_messages.append(parsed_response)
                print("[GDB terminal] "+str(cmd.timestamp)+" "+str(parsed_response.value).rstrip(),end='')
    ...
    def load(self):
        """
        Requires a file to be set first. Loads binary to target uC.
    ...
    def set_watch(self, value, ignore_error=False):
        """
        Sets a new watchpoint on the binary currently being debugged

        :param value: a variable name or address pointer
        :type value: str
        :returns: a :class:`CommandResult` instance
        :rtype: :class:`CommandResult`
        """
    ...
    def del_break(self, number):
        """
        Deletes a breakpoint by its number. If none provided
        all will be deleted.
        :param number: the breakpoint number
        :type number: int
        :returns: a :class:`CommandResult` instance
    ...
    def continue_(self):
        """
        Continues the application execution inside the debugger
