...
=item B<-image> I<elf-file>

Use I<elf-file> sections as image of the application to emulate. It is 
the same file used to program the flash via a JTAG/SWD programmer.

=item B<-board [type=]>I<name>

Select the emulated board by I<name>. Use C<-board help> to list
...
MCU can be used during emulation if C<-mcu> is added.

If not specified, a default board is used, and C<-mcu> becomes mandatory.

=item B<-mcu> I<model>

Select MCU model (C<-mcu help> for list and additional feature selection). 
...
=item B<-semihosting-cmdline>

The
C<cmdline> defines the entire command line passed to the application via the
